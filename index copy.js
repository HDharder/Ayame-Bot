...
async function carregarDadosPlanilha() {
  ...
  if (indiceAtualReal >= 0) {
      await sheet.loadCells(`A1:${sheet.getCell(0, sheet.columnCount - 1).a1Address.replace(/[0-9]/g, '')}1`);
      colunaAtualLetra = sheet.getCell(0, indiceAtualReal).a1Address.replace(/[0-9]/g, '');
  }
  await sheet.loadCells(`A2:${colunaAtualLetra}${sheet.rowCount}`);
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (commandName === 'abrir-mesa') {
      ...
      try {
        if (!interaction.member.roles.cache.some(role => role.name.toLowerCase() === 'narrador')) {
          await interaction.editReply({ content: 'Você precisa ter o cargo "Narrador" para usar este comando.', flags: [MessageFlagsBitField.Flags.Ephemeral] });
          return;
        }
        ...
        if (emoteAnimado) emoteId = emoteAnimado[1];
          ...
          await interaction.editReply({ content: 'Não consegui identificar esse emote.', flags: [MessageFlagsBitField.Flags.Ephemeral] });
          return;
        }
        ...
        await mensagemAnuncio.react(emoteString).catch(reactError => {
          ...
          interaction.followUp({ content: 'Não consegui usar esse emote para reagir.', flags: [MessageFlagsBitField.Flags.Ephemeral] }).catch(console.error);
        });
        ...
        if (interaction.deferred || interaction.replied) {
            await interaction.editReply({ content: `Ocorreu um erro ao abrir a mesa: ${error.message}`, components: [] }).catch(console.error);
             ...
             await interaction.reply({ content: `Ocorreu um erro ao abrir a mesa: ${error.message}`, flags: [MessageFlagsBitField.Flags.Ephemeral] }).catch(console.error);
         }
      }
    }
  ...
  if (interaction.isButton()) {
    try {
      // Adiciona deferUpdate() no início para evitar "Unknown Interaction"
      await interaction.deferUpdate();

      const [action, mestreInteractionIdOrMessageId, emoteId] = interaction.customId.split('|');
      if (action !== 'show_sort_modal') {
        const originalInteractionUserId = interaction.message.interaction?.user?.id;
        const isMestre = originalInteractionUserId ? (originalInteractionUserId === interaction.user.id) : false;
        const isStaff = interaction.member.roles.cache.some(role => role.name.toLowerCase() === 'staff');
        if (!isMestre && !isStaff) {
          await interaction.followUp({ content: 'Você não tem permissão para usar este botão.', flags: [MessageFlagsBitField.Flags.Ephemeral] });
          return;
        }
      }
      if (action === 'fechar_inscricao') {
        ...
        await interaction.showModal(modal); // showModal funciona após deferUpdate
      }
      else if (action === 'show_sort_modal') {
        const modal = new ModalBuilder()
          .setCustomId(`level_sort_modal|${interaction.message.id}`)
      ...
      // Sempre usa followUp ephemeral no catch após deferUpdate
