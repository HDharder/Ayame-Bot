...
module.exports = {
  ...
  buttons: [
    ...
    'relatorio_add_erva',
    'relatorio_add_pocao',
    'relatorio_add_misc', // <<< CORRIGIDO
    'relatorio_add_gold', // <<< CORRIGIDO
  ...
  async execute(interaction) {
    ...
    try {
        // <<< CORREÇÃO: Define isStaff ANTES de usar >>>
        ...
        const options = {
          ...
          goldTotal: interaction.options.getNumber('gold_total'), // Pode ser null
        ...
        interaction.client.pendingRelatorios.set(interaction.id, {
          ...
          isStaffExecutor: isStaff // Usa a variável que já definimos
        });
    ...
    if (action === 'relatorio_select_mesa') {
      try {
          ...
          state.players = playerInfos.map(pInfo => ({
              ...
              items: [],
              itemsData: {}, // Inicializa para modais
              extraGold: 0 // Inicializa gold extra
          }));
          ...
          // Verifica se precisa pedir drops (ou gold extra, que sempre é uma opção)
          ...
          // <<< ALTERADO: Mesmo sem drops, mostramos o input de Gold Extra >>>
          if (needsDropInput || !state.options.naoRolarLoot) {
              ...
              // Adiciona o botão de Gold Extra (sempre, a menos que o loot esteja ignorado)
              ...
              // <<< CORRIGIDO: Suporte para Múltiplas Linhas de Botões >>>
              const row1 = new ActionRowBuilder().addComponents(dropButtons.slice(0, 5));
              const row2Components = dropButtons.slice(5); // Pega 6º botão (Gold Extra) e seguintes
              ...
              await interaction.editReply({
                  ...
                  components: [row1, row2] // <<< CORRIGIDO
              });
  ...
  async handleModal(interaction) {
    ...
    // +++ NOVO: Modal de Gold Extra +++
      ...
      // Remonta a string de drops (copiado da lógica acima)
      let currentDropsString = "**Drops Adicionados para este jogador:**\n";
      let playerHasDrops = false;
      const dropTypes = { itens:'Itens', materiais:'Materiais', ervas:'Ervas', pocoes:'Poções', misc:'Misc'};
      for(const key in dropTypes) {
  ...
  async handleButton(interaction) {
    ...
    try {
        ...
        if (action.startsWith('relatorio_add_')) { 
          ...
          else if (action === 'relatorio_add_pocao') { itemType = 'pocao'; modal = new ModalBuilder().setCustomId(`modal_relatorio_${itemType}|${originalInteractionId}|${playerIndex}`).setTitle(`Adicionar Poções para ${state.players[playerIndex].tag}`); } // <<< CORRIGIDO
          ...
          // <<< CORRIGIDO: Não adiciona 'drop_input' ao modal de gold >>>
        ...
        // --- Botão Próximo Jogador / Finalizar Drops ---
            ...
            if (state.currentPlayerIndex < state.players.length) {
                ...
                // <<< CORRIGIDO: Lógica de múltiplas linhas para 'nextDropButtons' >>>
                ...
                await interaction.editReply({
                    ...
                    components: [nextRow1, nextRow2] // <<< CORRIGIDO
                });
